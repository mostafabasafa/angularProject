...
export class AccountService{
  ...
  paginateProvinceTest(page: number, limit: number){
    return this.http.get<User>(this.API_URL_PROVICE_TEST+'?_page='+ page +'&_limit='+ limit)
  }
  ...
  paginateAllCityTest(page: number, limit: number){
    return this.cityHTTP.get<City[]>(this.API_URL_CITY_TEST+'?_page='+ page +'&_limit='+ limit)
  }

  setProvinceTest(province: ProvinceArea){
    // console.log(city)
    return this.provinceHTTP.post(this.API_URL_PROVICE_TEST, province)
  }


  getProvinceTestById(id: number | string){
    return this.provinceHTTP.get<ProvinceArea>(this.API_URL_PROVICE_TEST+'/'+id)
  }

  updateProvince(id: number, params: any){
    return this.http.put(this.API_URL_PROVICE_TEST+'/'+id, params)
      .pipe(map(x  => {
